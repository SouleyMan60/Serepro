...
export default function UserInfoCard() {
  ...
  const [prenom, setPrenom] = useState(fullName.split(" ")[0] ?? "");
  const [nom, setNom] = useState(fullName.split(" ").slice(1).join(" ") ?? "");
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
            <div>
              ...
              <p className="text-sm font-medium text-gray-800 dark:text-white/90">{prenom || "—"}</p>
            </div>
            <div>
              <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Nom</p>
              <p className="text-sm font-medium text-gray-800 dark:text-white/90">{nom || "—"}</p>
            </div>
            <div>
              <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Adresse email</p>
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          ...
          <div className="px-2 pb-3">
            <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
              ...
              <div>
                <Label>Nom</Label>
                <Input type="text" value={nom} onChange={(e) => setNom(e.target.value)} placeholder="Kouassi" />
              </div>
              <div>
                <Label>Adresse email</Label>
                <Input type="email" value={email} disabled placeholder="contact@exemple.ci" />
              </div>
              <div>
              ...
              <div className="col-span-2">
                ...
                <Input type="text" value={bio} onChange={(e) => setBio(e.target.value)} placeholder="Ex : Directeur RH, ACME CI" />
              </div>
            </div>
            {saveError && (
